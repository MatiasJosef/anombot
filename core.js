...
function checkUserApplication(message) {
    ...
    const form = ["1) Ваш игровой ник:", "2) Доказательство наличия у вас VIP (Скриншот !vip):"];
        ...
        switch (messageReaction.emoji.name) {
            case "✅": {
                ...
                if (!nickname || nickname.length === 0) return messageReaction.message.channel.send(user, new Discord.MessageEmbed()
                    .setColor(0xe53935)
                    .setDescription(`Пользователю ${messageReaction.message.author} не удалось изменить роль (скорее всего, неправильно заполнен формуляр)`));
                ...
                try {
                    ...
                    if (error.message === "Missing Permissions") {
                        return messageReaction.message.channel.send(user, new Discord.MessageEmbed()
                            .setColor(0xe53935)
                            .setDescription(`У бота недостаточно прав для изменения никнейма/роли пользователя ${messageReaction.message.author}`));
                    }
                }

                await responseMessage(messageReaction.message, new Discord.MessageEmbed()
                    .setColor(0x43a047)
                    .setDescription(`Ваше [заявление](${messageReaction.message.url}) на изменение роли было одобрено модератором ${user}`));
                break;
            }
            case "❎": {
                await responseMessage(messageReaction.message, new Discord.MessageEmbed()
                    .setColor(0xe53935)
                    .setDescription(`Ваше [заявление](${messageReaction.message.url}) на изменение роли было отклонено модератором ${user}`));
