            ...
            switch (reaction.emoji.name) {
                case '✅': {
                    const embed = new Discord.MessageEmbed()
                        .setColor(0x43a047)
                        .setDescription(`Ваше [заявление](${message.url}) на изменение роли было одобрено модератором ${user}`);
                    try {
                ...
                case '❎': {
                    const embed = new Discord.MessageEmbed()
                        .setColor(0xe53935)
                        .setDescription(`Ваше [заявление](${message.url}) на изменение роли было отклонено модератором ${user}`);
                    try {
                        await message.member.send(embed);
                    } catch (err) {
                        await message.channel.send(message.author, embed);
